...
# ===============================
@torch.inference_mode()
def evaluate_video_mri(predictor, npz_path, out_dir):
    data = np.load(npz_path)
    imgs = data["imgs"]
    video_name = os.path.splitext(os.path.basename(npz_path))[0]
    ...
    # 2️⃣ init state
    state = predictor.init_state(video_path=tmp_dir)
    # ✅ [ADD] 반드시 프롬프트를 먼저 넣어야 propagate 가능
    add_center_box_prompt(
        predictor,
        state,
        imgs[0].shape
    )
    ...
    # 3️⃣ propagate from frame 0
    for fidx, obj_ids, logits in predictor.propagate_in_video(state, start_frame_idx=0):
...
def add_center_box_prompt(predictor, state, img_shape):
    H, W = img_shape
...
def main():
    ...
    parser.add_argument("--cfg", default="configs/sam2.1_hiera_t512.yaml")
    parser.add_argument("--data_root", required=True)
    parser.add_argument("--out_root", default="./eval_results_MRI")
    args = parser.parse_args()
    ...
    for fn in sorted(os.listdir(args.data_root)):
        ...
        vname = evaluate_video_mri(
            predictor,
            os.path.join(args.data_root, fn),
            out_dir
        )
        print(f"[MRI] Inference done: {vname}")

    print("\n✅ MRI inference-only finished")
    print(f"🖼 Results saved to: {out_dir}/vis")
